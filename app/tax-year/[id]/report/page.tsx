...
import Form1040Editor from "@/components/irs/Form1040Editor";
      ...
      <div className="max-w-5xl mx-auto px-4 py-8">
        ...
        <div className="mt-6 flex items-start justify-between">
          <div>
            ...
            <p className="text-gray-400 text-sm mt-1">For planning purposes only — not a filed return</p>
          </div>
          <a
            href={downloadUrl}
            download
        ...
        {error && (
          <div className="mt-6 bg-red-900/30 border border-red-700 rounded-lg p-4 text-red-300">{error}</div>
        )}
        ...
        {report && (
          <div className="mt-6 space-y-6">
            ...
            <div className="bg-gray-900 rounded-xl border border-gray-800 p-6">
              <div className="flex items-center justify-between">
                ...
                <div className="text-right">
                  ...
                  <div className="text-sm text-gray-300">{new Date(report.generatedAt).toLocaleDateString()}</div>
                </div>
              </div>
            </div>
            ...
            <div className="grid grid-cols-2 md:grid-cols-3 gap-4">
              {[
                ...
                <div key={c.label} className="bg-gray-900 border border-gray-800 rounded-xl p-5">
                  ...
                  <div className={`text-2xl font-bold ${c.color}`}>${fmtDollar(c.value)}</div>
                </div>
              ))}
            </div>
                  ...
                  .map(([num, label, value]) => {
                    ...
                    return (
                      <div key={num} className={`flex items-center justify-between px-6 py-3 ${isKey ? "bg-gray-800/50" : ""}`}>
                        ...
                        <span className={`font-mono text-sm font-semibold ${num === "Line 37" ? "text-red-400" :
            ...
            {/* Form 1040 Editor */}
            <div className="bg-gray-900 rounded-xl border border-yellow-800/40 overflow-hidden">
              <div className="px-6 py-4 border-b border-gray-800 flex items-center justify-between">
                <div>
                  <h2 className="font-semibold text-white">Form 1040 — Manual Review ✍️</h2>
                  <p className="text-xs text-gray-500 mt-0.5">Compare system values with your tax preparer's numbers</p>
                </div>
                <span className="text-xs bg-yellow-900/40 border border-yellow-700/50 text-yellow-300 px-2 py-1 rounded">
                  Task 8 — Beta
                </span>
              </div>
              <div className="p-6">
                <Form1040Editor report={report} />
              </div>
            </div>
            ...
            <div className="bg-yellow-950/30 border border-yellow-800/50 rounded-xl p-4">
              <p className="text-xs text-yellow-300/80">{report.notes?.join(" ")}</p>
            </div>
          </div>
        )}
      </div>
